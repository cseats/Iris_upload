...
def asset_risk(all_sites,asset_hazard_conseq,auth_dict,assessment_type,_risk_key):
    ...
    for asset_indx in range(len(all_sites[_risk_key])):
        ...
        for haz in asset_hazard_conseq.keys():
            ...
            # print("this is the assessment type")
            # print(assessment_type)
            # print(_o)
            if haz == "hydrological_flooding":
                drive = all_sites["Flood Driver "+assessment_type["year"]][asset_indx]
                driver = drive.replace("'","")
                driver = driver.replace("[","")
                driver = driver.replace("]","")
                driver = "The driving source of flood is "+driver + " flooding."

                # print("THIS IS THE DRIVERRR")
                # print(driver)
                # print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
                # raise Exception(f">> Failed to add risk ratings")   
            else:
